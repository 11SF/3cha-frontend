...
/** Returns the next working day (Mon–Fri, not a holiday) starting from the day after `from`. */
...
function AvatarCircle({ name, color, size = 'md' }: { name: string; color: string; size?: 'sm' | 'md' | 'lg' }) {
  ...
  return (
    <div
      className={cn('rounded-full flex items-center justify-center font-bold text-white flex-shrink-0', sizeClass)}
...
export function QueueBoardPage() {
  ...
  // Build holiday set and find next actual working day.
        ...
        <Card className={cn('transition-all', !isPending && 'opacity-75')}>
          ...
          <CardContent>
            <div className="flex items-center gap-4">
              <AvatarCircle name={entry.memberName} color={entry.avatarColor} size="lg" />
              <div>
